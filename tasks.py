...
class ProjectTeamTasks:
    ...
    def design_audit_task(self, designer, code_content):
        """[SUPERVISOR LOOP] Execute Professional UI/UX Audit"""
        return Task(
            description=f"""[SUPERVISOR: UI/UX DESIGN AUDIT]
            
            Target Code:
            {code_content}
            
            [OPERATIONAL DIRECTIVE]
            Perform a rigorous, high-end UI/UX audit of the provided code as a Senior Art Director [Claude 4.6].
            Analyze based on:
            1. **Visual Hierarchy**: Does the eye flow naturally to the CTA?
            2. **Aesthetics (2026 SOTA)**: Is it "Premium" or "Generic"?
            3. **Accessibility**: Contrast ratios, font sizes, touch targets.
            4. **User Experience**: Is the flow simple? Are there friction points?
            
            [APPROVAL GATE]
            You MUST return a decision:
            - **STATUS: APPROVED**: If and only if the design is 100/100.
            - **STATUS: REJECTED**: If there is even a 1% improvement possible.
            
            If REJECTED, provide a numbered list of 'Execution Directives' for the Coder (Codex).
            Be brutal. Be precise. No pleasantries.
            """,
            expected_output="""A professional audit report with a clear STATUS: APPROVED or REJECTED and specific improvement points.""",
            agent=designer,
        )
